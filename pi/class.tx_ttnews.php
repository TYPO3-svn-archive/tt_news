 ...
 *
 *
 *   84: class tx_ttnews extends tslib_pibase
 *  113:     function main_xmlnewsfeed($content, $conf)
 *  128:     function getStoriesResult()
 *  139:     function init($conf)
 *  302:     function main_news($content, $conf)
 *  365:     function newsArchiveMenu()
 *  490:     function displaySingle()
 *  553:     function displayList()
 *  776:     function getListContent($itemparts, $selectConf, $prefix_display)
 *  816:     function getSelectConf($where, $noPeriod = 0)
 *  910:     function initCategories()
 *  940:     function generatePageArray()
 *  956:     function getItemMarkerArray ($row, $textRenderObj = 'displaySingle')
 * 1054:     function getCatMarkerArray($markerArray, $row, $lConf)
 * 1145:     function getImageMarkers($markerArray, $row, $lConf, $textRenderObj)
 * 1198:     function getRelated($uid)
 * 1253:     function userProcess($mConfKey, $passVar)
 * 1268:     function spMarker($subpartMarker)
 * 1286:     function searchWhere($sw)
 * 1298:     function formatStr($str)
 * 1313:     function getLayouts($templateCode, $alternatingLayouts, $marker)
 * 1331:     function getXmlHeader()
 * 1389:     function getNewsSubpart($myTemplate, $myKey, $row = Array())
 *
 * TOTAL FUNCTIONS: 22
			...
			if (isset($this->categories[$row['uid']]) && ($this->config['catImageMode'] || $this->config['catTextMode'])) {
				...
				while (list ($key, $val) = each ($this->categories[$row['uid']])) {
					...
					if ($this->config['catTextMode'] == 0) {
						...
						$news_category[] = $this->pi_linkTP_keepPIvars($catTitle, array('cat' => $this->categories[$row['uid']][$key]['catid'], 'backPid' => null, 'pointer' => null), '', '', ($this->config['catSelectorTargetPid']?$this->config['catSelectorTargetPid']:$GLOBALS['TSFE']->id));
					}
